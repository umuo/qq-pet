...
function callOpenAICompatible(apiKey, messages) {
  return new Promise((resolve, reject) => {
    const model =
      (typeof getSys === "function" && getSys("llmModel")) || DEFAULT_MODEL;
    const rawUrl =
      (typeof getSys === "function" && getSys("llmUrl")) || DEFAULT_URL;
    const parsedUrl = parseEndpoint(rawUrl);
...
class LLMService {
  ...
  test(apiKey, petInfo) {
    return callOpenAICompatible(apiKey || "", [
      { role: "system", content: SYSTEM_PROMPT(petInfo || {}) },
