const _require = eval("require");
const path = _require("path");
class mainClass {
  constructor(t) {
    this.window = null;
    this.show = false;
    this.name = "tip";
    this.closeTime = 10000;
    this.closeTimeOut = null;
  }
  nowPosition = [0, 0];
  cleate(t) {
    this.nowPosition = t.position || [0, 0];
    this.maxSize = t.maxSize || [0, 0];
    this.width = 229;
    this.height = 179;
    let e = this;
    let s = [0, 0];
    s[0] = Math.trunc(t.position[0] + t.maxSize[0] / 2 - this.width / 2);
    s[1] = Math.trunc(t.position[1] - this.height - 30);
    this.msgData = t.data;
    this.otherMsgData = t.otherOpt || null;
    windowsMain.open({
      name: this.name,
      loadFile: "popups/" + this.name,
      default: {
        width: this.width,
        focusable: false,
        show: false,
        height: this.height,
        x: s[0] || 1,
        y: s[1] || 1
      },
      created(t) {
        let {
          vm: s,
          preloads: i,
          getinfo: o,
          wsMethods: a
        } = t;
        s.setAlwaysOnTop(true);
        const h = t => {
          if (!s?.webContents?.send) {
            return;
          }
          let i = {
            size: [229, 179]
          };
          if (t.otherOptions?.sweetHeart) {
            i = {
              size: [271, 179]
            };
          }
          if (i.size[0] != e.width || i.size[1] != e.height) {
            e.setPosition({
              position: e.nowPosition,
              maxSize: e.maxSize
            }, i.size[0], i.size[1]);
          }
          s.webContents.send("tip_m_setData_h", {
            type: "otherOptions",
            data: t.otherOptions
          });
        };
        i({
          tip_h_say_m: (t, e) => {
            console.log(e, " --- tip_h_say_m say");
          },
          tip_h_bus_m: (t, i) => {
            if (i.event == "mounted") {
              e.setMsg({
                data: e.manageMsg(e.msgData),
                otherOpt: e.otherMsgData
              });
              h(getPetInfo());
              s.webContents.send("tip_m_bus_h", {
                type: "load"
              });
            } else if (i.event == "close") {
              if (i.data) {
                let t = i.data;
                try {
                  t = JSON.parse(t);
                } catch (t) {}
              }
              if (e.otherMsgData) {
                console.log("_this.otherMsgData", e.otherMsgData);
                if (e.otherMsgData?.type == "click") {
                  if (e.otherMsgData?.fn) {
                    e.otherMsgData?.fn();
                  }
                } else if (e.otherMsgData?.mood) {
                  addPetInfo(e.otherMsgData);
                }
              }
              e.doClose();
            }
          }
        });
        o([{
          event: "pet",
          name: e.name,
          fn: t => {
            h(t);
          }
        }]);
      },
      onload() {
        console.log("onload ", e.name);
        e.show = true;
      },
      onshow(t) {
        console.log("onshow ", e.name);
        e.window = t;
        e.show = true;
      },
      onhide() {
        console.log("onhide ", e.name);
        e.show = false;
      },
      onclose() {
        console.log("onclose ", e.name);
        if (e.closeTimeOut) {
          clearTimeout(e.closeTimeOut);
        }
        e.window = null;
        e.show = false;
      }
    }).then(t => {
      this.window = t;
      this.init();
    }).catch(t => {
      console.log(t);
    });
  }
  init() {
    this.show = true;
  }
  setPosition(t, e, s) {
    if (t?.position?.length != 2) {
      return;
    }
    this.nowPosition = t.position || [0, 0];
    this.maxSize = t.maxSize || [0, 0];
    if (e) {
      this.width = e;
    }
    if (s) {
      this.height = s;
    }
    let i = [0, 0];
    i[0] = Math.trunc(t.position[0] + t.maxSize[0] / 2 - this.width / 2);
    i[1] = Math.trunc(t.position[1] - this.height - 30);
    let o = {
      x: i[0] || 1,
      y: i[1] || 1,
      height: this.height + 10,
      width: this.width + 10
    };
    this.window.setBounds(o);
  }
  setMsg(t, e) {
    if (this.window) {
      this.msgData = t.data;
      this.otherMsgData = t.otherOpt || null;
      this.window.webContents.send("tip_m_bus_h", {
        data: JSON.stringify(this.manageMsg(this.msgData)),
        type: "nextData"
      });
      if (this.closeTimeOut) {
        clearTimeout(this.closeTimeOut);
      }
      this.closeTimeOut = setTimeout(() => {
        this.doClose();
      }, t?.closeTime || this.closeTime);
    }
  }
  manageMsg(t) {
    if ((t.type == "text" || t.type == "seeTextImgs") && !!t.data && t.data?.form != "clip") {
      if ((t.data + "")?.indexOf("[host]") != -1) {
        t.data = (t.data + "").replace(/\[host\]/g, getPetInfoOne("host", "info"));
      }
    }
    return t;
  }
  doClose() {
    if (global.nextActive) {
      global.nextActive({
        name: "speak"
      });
    }
    if (this.closeTimeOut) {
      clearTimeout(this.closeTimeOut);
    }
    this.window.close();
    this.show = false;
  }
}
let main = new mainClass();
module.exports = main;
