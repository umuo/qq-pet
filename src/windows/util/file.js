const _require = eval("require"),
      ...
      n = (r) => {
        fs.access(r, fs.constants.F_OK | fs.constants.W_OK, function (o) {
          o
            ? "ENOENT" === o.code
              ? addFolder(r, (e) => {
                  "success" == e.msg ? n(r) : l({ event: "error", msg: e.msg });
                })
            ...
            : (() => {
                for (let n in e)
                  try {
                    ...
                    fs.existsSync(t)
                      ? l({ event: "error", msg: "该文件已存在", val: e[n] })
                      : o();
                  } catch (r) {
                    l({
                      event: "error",
                      msg: "move" == t ? "文件移动错误" : "文件复制错误",
                      val: { ...e[n], error: r },
                    });
                  }
              })();
  ...
  getAllFolder = function (e, r, t) {
    let l = e.replace(/\\/g, "/"),
      s = l.split("/"),
      n = "";
    (s[s.length - 1] && (t || s.splice(s.length - 1, 1), (l = s.join("/"))),
  ...
  reFileName = function (e, r) {
    ...
    if (fs.existsSync(l)) r({ event: "error", msg: "该文件已存在", val: s });
    else
      try {
        n();
      } catch (e) {
